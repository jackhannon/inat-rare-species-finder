...
function createLink(url: string, txt: string | number = url) {
	return '<a href="' + url + '">' + String(txt) + "</a>";
}
function createAmp(str: string) {
	return str.replace(/&/g, "&amp;");
}
function comNum(n: number) {
	...
	for (const [key, value] of Object.entries(attributes)) {
		if (typeof value === "object" && value !== null) {
			for (const [subkey, subvalue] of Object.entries(value)) {
				(obj as any)[key][subkey] = subvalue;
			}
		} else {
...
function addElems({
	...
	type: string;
	parent?: HTMLElement;
	attributes?: { [key: string]: string | undefined | number }[];
}) {
...
function constructTable(species: typeof queryResults) {
...
function appendSpecies(species: typeof queryResults, tbody: HTMLElement) {
	for (let i = 0; i < species.length; i++) {
		...
		let url_explore =
			"https://www.inaturalist.org/observations?taxon_id=" + rec.taxon.id;
		let obsparam_verifiable =
			obsparams.get("verifiable") === null
				? "&verifiable=any"
		...
		let values = [
			{ innerText: i + 1 },
			{
				innerHTML: createLink(
					"https://www.inaturalist.org/taxa/" + rec.taxon.id,
					rec.taxon.id
				),
			},
			{
			...
			},
			{
				classList: "tar",
				innerHTML: createLink(
					url_explore + "&verifiable=any",
					comNum(rec.taxon.observations_count)
				),
			},
		];
...
function filterByTotalObservationsDifference(sliderPercentage: number) {
...
const handleContainerMouseMove = (e: MouseEvent) => {
	if (
		document.querySelector(".sliderContainer")?.getAttribute("data-dragging")
	) {
		updateSliderFillByEvent("--slider-fill", e);
	}
};

const updateSliderFillByEvent = (variableName: string, e: MouseEvent) => {
	...
	if (elem) {
		...
		(
			document.querySelector(".sliderContainer") as HTMLElement
		).style.setProperty(variableName, `${fillWidth}%`);
		return fillWidth;
	}
};

function computeCurrentWidthFromPointerPos(
	xDistance: number,
	left: number,
	totalWidth: number
) {
...
function onChangeEndemismSlider(sliderPercentage: number) {
...
const resultsPerPage =
	Number(getSingleQueryParam(urlParams, "per_page")) || 2000;
...
let urlsToMakeRequestsTo: string[] = [];
...
const apiURL =
	apiBase + (urlParams.toString() != "" ? "?" + urlParams.toString() : "");
const apiReferenceURL =
	"http://api.inaturalist.org/v1/docs/#!/Observations/get_observations_species_counts";
...
if (urlParams.toString() === "") {
	let instructions = [
		{
			innerHTML:
				"This page displays the response from the " +
				apiReference +
		...
		{
			innerHTML:
				"Suppose the address of this page is " +
				apiBase +
	...
	Promise.all(urlsToMakeRequestsTo.map((url) => fetch(url)))
		...
		.catch((err) => {
			...
			addElem({
				...
				attributes: {
					innerText:
						"There was a problem retrieving data. Error " + err.message + ".",
				},
			});
		});
}
